# from fastapi import APIRouter
# from bson.son import SON
# from app.config.database import outbreak_collection
# from datetime import datetime

# router = APIRouter()


# @router.get("/daily-trends")
# # async def get_daily_trends():
# #     pipeline = [
# #         {
# #             "$group": {
# #                 "_id": {
# #                     "$dateToString": {
# #                         "format": "%Y-%m-%d",
# #                         "date": "$timestamp"
# #                     }
# #                 },
# #                 "total_cases": {"$sum": 1}
# #             }
# #         },
# #         {"$sort": SON([("_id", 1)])}
# #     ]
# #     results = await outbreak_collection.aggregate(pipeline).to_list(length=None)
# #     return {"daily_trends": results}
# @router.get("/daily-trends")
# async def get_daily_trends():
#     pipeline = [
#         {
#             "$group": {
#                 "_id": {
#                     "$dateToString": {
#                         "format": "%Y-%m-%d",
#                         "date": "$timestamp"
#                     }
#                 },
#                 "total_cases": {"$sum": 1}
#             }
#         },
#         {"$sort": SON([("_id", 1)])}
#     ]

#     results = await outbreak_collection.aggregate(pipeline).to_list(length=None)

#     # Convert each _id (YYYY-MM-DD) to "Month Day" format
#     formatted = [
#         {
#             "date": datetime.strptime(item["_id"], "%Y-%m-%d").strftime("%b %d"),
#             "cases": item["total_cases"]
#         }
#         for item in results
#     ]

#     # Separate into labels and values
#     labels = [item["date"] for item in formatted]
#     values = [item["cases"] for item in formatted]

#     return {"labels": labels, "values": values}



# @router.get("/weekly-trends")
# async def get_weekly_trends():
#     pipeline = [
#         {
#             "$group": {
#                 "_id": {
#                     "year": {"$isoWeekYear": "$timestamp"},
#                     "week": {"$isoWeek": "$timestamp"}
#                 },
#                 "total_cases": {"$sum": 1}
#             }
#         },
#         {"$sort": SON([("_id.year", 1), ("_id.week", 1)])}
#     ]
#     the_results = await outbreak_collection.aggregate(pipeline).to_list(length=None)

#     formatted = [
#         {
#             "week": f"{item['_id']['year']}-W{item['_id']['week']:02}",
#             "total_cases": item["total_cases"]
#         }
#         for item in the_results
#     ]

#     return {"weekly_trends": formatted}
from fastapi import APIRouter
from bson.son import SON
from app.config.database import outbreak_collection
from datetime import datetime, timedelta

router = APIRouter()

@router.get("/daily-trends")
async def get_daily_trends():
    # Aggregate total cases per day from MongoDB
    pipeline = [
        {
            "$group": {
                "_id": {
                    "$dateToString": {
                        "format": "%Y-%m-%d",
                        "date": "$timestamp"
                    }
                },
                "total_cases": {"$sum": 1}
            }
        },
        {"$sort": SON([("_id", 1)])}
    ]

    results = await outbreak_collection.aggregate(pipeline).to_list(length=None)

  

    labels = [datetime.strptime(item["_id"], "%Y-%m-%d").strftime("%d") for item in results]
    values = [item["total_cases"] for item in results]

    return {"labels": labels, "values": values}